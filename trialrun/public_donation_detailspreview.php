...
require_once("include/public_donation_variables.php");
...
$layout = new TLayout("detailspreview", "ExtravaganzaOffice", "MobileOffice");
$layout->version = 2;
...
$page_layouts["public_donation_detailspreview"] = $layout;
...
if($mastertable == "public.users")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("donor") . "=" . make_db_value("donor",$_SESSION[$strTableName."_masterkey1"]);
}
if($mastertable == "public.project")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("project") . "=" . make_db_value("project",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["time"]));
		$keylink.="&key2=".runner_htmlspecialchars(rawurlencode(@$data["donor"]));
		$keylink.="&key3=".runner_htmlspecialchars(rawurlencode(@$data["project"]));
	...
	//	donor - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("donor", $data, $keylink);
			$row["donor_value"] = $value;
			$format = $pSet->getViewFormat("donor");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("donor")))
				$class = ' rnr-field-number';
			$row["donor_class"] = $class;
	//	amount - Number
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("public_donation", "detailspreview"));
