...
require_once("include/public_users_variables.php");
...
$layout = new TLayout("register2", "ExtravaganzaOffice", "MobileOffice");
$layout->version = 2;
...
$layout = new TLayout("register_success2", "ExtravaganzaOffice", "MobileOffice");
$layout->version = 2;
$layout->blocks["top"] = array();
$layout->containers["register"] = array();
$layout->container_properties["register"] = array(  );
...
//	processing name
	...
	$control_name->readWebValue($values, $blobfields, NULL, NULL, $filename_values);
...
//	processing phone
	...
	$control_phone->readWebValue($values, $blobfields, NULL, NULL, $filename_values);
...
//	processing gender
	...
	$control_gender->readWebValue($values, $blobfields, NULL, NULL, $filename_values);
...
//	check if entered username already exists
	...
	{
		$strSQL = "select count(*) from ". $pageObject->connection->addTableWrappers( "public.users" ) 
			. " where " . 
	...
	if($retval)
	{
		$retval = DoInsertRecord("public.users", $values, $blobfields, $pageObject);
	}
