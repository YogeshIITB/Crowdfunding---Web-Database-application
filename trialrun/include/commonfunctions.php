...
function checkTableName($shortTName, $type=false)
{
	...
	if ("public_donation" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("public_users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("public_project" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("public.donation");
	...
	$strPerm = GetUserPermissions("public.project");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="public.project";
	}
...
function GetUserPermissionsStatic( $table )
{
	...
	if($table=="public.donation" && $sUserGroup=="Y")
	{
			return "AEDS".$extraPerm;
	}
//	default permissions	
	if($table=="public.donation")
	{
		return "AS".$extraPerm;
	}
	if($table=="public.users" && $sUserGroup=="Y")
	{
			return "AEDS".$extraPerm;
	}
//	default permissions	
	...
	if($table=="public.project" && $sUserGroup=="Y")
	{
			return "AEDS".$extraPerm;
	}
//	default permissions	
	if($table=="public.project")
	{
		return "AEDS".$extraPerm;
	}
...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("public.users")
...
{
	global $globalEvents;
	$_SESSION["GroupID"] = $data["admin"];
	if($_SESSION["GroupID"]=="Y")
		$_SESSION["AccessLevel"] = ACCESS_LEVEL_ADMINGROUP;


		$_SESSION["OwnerID"] = $data["username"];
	$_SESSION["_public.donation_OwnerID"] = $data["username"];
		$_SESSION["_public.users_OwnerID"] = $data["username"];
		$_SESSION["_public.project_OwnerID"] = $data["username"];
